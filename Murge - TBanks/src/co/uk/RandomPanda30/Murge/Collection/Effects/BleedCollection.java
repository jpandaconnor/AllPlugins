...
public class BleedCollection implements EffectsBase {
	...
	public void playEffect(Player player) throws NoSuchMethodException,
		...
		Class<?> packetClass = ReflectionHandler.getHandler().getNMSClass(
				"PacketPlayOutWorldParticles");
		Constructor<?> packetConstructor = packetClass.getConstructor(
				net.minecraft.server.v1_8_R2.EnumParticle.class, boolean.class,
		...
		for (Entity entity : location.getWorld().getEntities()) {
			if (entity instanceof Player) {
				if (location.distance(entity.getLocation()) < 333) {
					...
					Object packet = packetConstructor
							.newInstance(
									net.minecraft.server.v1_8_R2.EnumParticle.DRIP_LAVA,
									true, x, y, z, 0.5f, 1.0f, 0.5f, 100, 100,
									null);
					Method sendPacket = ReflectionHandler
							.getHandler()
							.getNMSClass("PlayerConnection")
							.getMethod(
									"sendPacket",
									ReflectionHandler.getHandler().getNMSClass(
											"Packet"));
					sendPacket.invoke(ReflectionHandler.getHandler()
							.getConnection(near), packet);
