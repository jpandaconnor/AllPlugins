...
public class DeathsCollection implements PlayerBase {
	...
	public int getValue(UUID uuid) {
		...
		return (Integer) Murge.cso.getDataC().get(uuid.toString() + ".deaths");
	}
	...
	public void addValue(UUID uuid) {
		if ((boolean) Murge.cMap.getStat(ConfigValues.MYSQL_ENABLE)) {
			try {
				mysql.updateSQL("UPDATE murge SET deaths = '"
			...
			Murge.cso.getDataC().set(uuid.toString() + ".deaths",
					getValue(uuid) + 1);
	...
	public void setValue(UUID uuid, int amount) {
		if ((boolean) Murge.cMap.getStat(ConfigValues.MYSQL_ENABLE)) {
			try {
				mysql.updateSQL("UPDATE murge SET deaths = '" + amount + "'"
						+ " WHERE uuid = '" + uuid.toString() + "'");
			} catch (ClassNotFoundException | SQLException e) {
				e.printStackTrace();
			}
