...
public enum Messages {
	...
	SM (new ArrayList<String>() {
		{
			add("%TAG %TLoading %AMurge");
			add("%T Made by RandomPanda30");
			add(" ");
			add("%TThank you for purchasing, enjoy!");
		}
	}),

	SHM (new ArrayList<String>() {
		{
			add("%TAG %TClosing %AMurge");
			add("%T Made by RandomPanda30");
			add(" ");
			add("%TThank you for purchasing, enjoy!");
		}
	...
	REGEN_MAYLAG ("%TAG %E&lThe server is regenerating ores + trees. Expect some lag!"),
	...
	HELP_BADSYNTAX ("%TAG %EError, something went wrong with this command. Do %A/murge list %Efor a list of commands"),
	HELP_COMMANDSLIST (new ArrayList<String>() {
		{

		}
	}),

	PURGE_ALREADYON ("%TAG %EThe purge is already active!"),
	PURGE_ALREADYOFF ("%TAG %EThe purge is not active!"),

	COMMAND_NOTENABLED ("%TAG %EThis command is not enabled in the config"),

	NOPERMISSION ("%TAG %EYou do not have permission to do this!"),

	WB_RESET ("%TAG %NThe world border has been reset"),
	WB_ALREADYDISABLED ("%TAG %EThe world border is already disabled"),

	LOCATIONS_ALREADYEDITING ("%TAG %EPlayer: %A%player %Eis already editing the location"),

	SETUP_ENTERKITNAME ("%TAG %NPlease open the chat and type the name of the kit you want to create"),
	SETUP_ENTERKITCOOLDOWN ("%TAG %NOk, now please enter the cooldown time you want for the kit. For example %A2d5h2m30s"),
	SETUP_KITMADE ("%TAG %NKit created!");
