...
public class PurgeOptionsItems {
	...
	public static ItemStack obtainBlockInProgress() {
		boolean c = MiscCollection.getCollection().isJoinBlockInProgress();
		String title = "";
		if (c) {
			title = "%NBlock joining whilst in progress: %GOn";
		} else {
			title = "%NBlock joining whilst in progress: %BOff";
		}

		ArrayList<String> lores = new ArrayList<String>();
		lores.add("%TBlock a player from joining the server");
		lores.add("%Twhen the purge is occuring?");
		lores.add(" ");
		lores.add("%TOps will be able to join regardless");
		return ItemMethods
				.createItem(title, Material.STONE_BUTTON, 1, 0, lores);
	}

	public static ItemStack obtainRegenOres() {
		boolean c = OreCollection.getCollection().isEnabled();
		String title = "";
		if (c) {
			title = "%NOre regeneration: %GOn";
		} else {
			title = "%NOre regeneration: %BOff";
		}

		ArrayList<String> lores = new ArrayList<String>();
		lores.add("%TStore and regenerate natural ores?");
		lores.add(" ");
		lores.add("%TThe ores will regenerate after the time");
		lores.add("%Tthat has been specified in the config");
		return ItemMethods
				.createItem(title, Material.STONE_BUTTON, 1, 0, lores);
	}

	public static ItemStack obtainRegenTrees() {
		boolean c = OreCollection.getCollection().isEnabled();
		String title = "";
		if (c) {
			title = "%NTree regeneration: %GOn";
		} else {
			title = "%NTree regeneration: %BOff";
		}

		ArrayList<String> lores = new ArrayList<String>();
		lores.add("%TStore and regenerate trees?");
		lores.add(" ");
		lores.add("%TThe trees will regenerate after the time");
		lores.add("%Tthat has been specified in the config");
		return ItemMethods
				.createItem(title, Material.STONE_BUTTON, 1, 0, lores);
	}
